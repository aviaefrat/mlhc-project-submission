...
LOG = utils.make_logger("labs_features", "labs_features.log")
...
def _load_inputs(chort_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    """
    Returns:
      labs_df: (subject_id, admission_id, feature_name, charttime, valuenum)
      cohort_df: (subject_id, admission_id, admittime)
      feature_names_all: ordered unique feature names from labs.csv
    """
    labs_df = pd.read_parquet(utils.LABS_PATH).loc[
        :, ["subject_id", "admission_id", "feature_name", "charttime", "valuenum"]
    ].copy()
    labs_df["charttime"] = pd.to_datetime(labs_df["charttime"], errors="coerce")
    labs_df["feature_name"] = labs_df["feature_name"].astype("string")

    cohort_df = pd.read_parquet(chort_path).loc[
    ...
    csv = pd.read_csv(utils.LABS_CSV)
    if "feature name" not in csv.columns:
        raise ValueError(f"{utils.LABS_CSV} must have a 'feature name' column")
    feature_names_all = (
        csv["feature name"].astype("string").drop_duplicates().tolist()
    )
    return labs_df, cohort_df, feature_names_all


def _constrain_to_first_48h(labs_df: pd.DataFrame, cohort_df: pd.DataFrame) -> pd.DataFrame:
    df = labs_df.merge(
...
def _compute_admission_level_features(
    ...
    """
    For each (subject_id, feature_name), compute admission-level stats.
    """
    nonna = df48.loc[
    ...
    for col in [
        ...
        p = adm.pivot(index="subject_id", columns="feature_name", values=col)
        p.columns = [f"{c}_{col}" for c in p.columns]
        wide_blocks[col] = p

    out = pd.concat(wide_blocks.values(), axis=1).reset_index()
    return out
...
def _compute_per_window_long(df48: pd.DataFrame, feature_names_all: list[str]) -> pd.DataFrame:
    ...
    stats_df = pd.concat(
        [agg.drop(columns=["stats"]), pd.DataFrame(agg["stats"].tolist(), index=agg.index)],
        axis=1,
    )
...
def _pivot_per_window_wide(ffilled_long: pd.DataFrame) -> pd.DataFrame:
    ...
    for col, suffix in [
        ...
        for (fname, win) in p.columns:
            win = int(win)
            flat.append(f"{fname}_window{win}_{suffix}")
        p.columns = flat
        pieces.append(p)
...
def _stable_column_order(
    ...
    def present(c: str) -> bool:
        return (c in adm_wide.columns) or (c in win_wide.columns) or (c in diff_wide.columns)
    ...
    existing = ["subject_id", "admission_id"]
    existing += [c for c in cols if c not in existing]
    existing = [c for c in existing if c in adm_wide.columns or c in win_wide.columns or c in diff_wide.columns]
...
def _log_feature_summaries(features: pd.DataFrame, feature_names_all: list[str]) -> None:
    ...
    for fname in feature_names_all:
        ...
        counts.append((fname, int(features[col].fillna(0).sum())) if col in features.columns else (fname, 0))
    counts.sort(key=lambda x: x[1], reverse=True)
    LOG.info(f"Top {TOP_N_FEATURES} lab features by total measurements in 0–48h:")
    ...
    LOG.info("Per-lab NA fractions (first/last/diff; window means after ffill: w0..w3):")
    for fname in feature_names_all:
        ...
        f_na = frac_na(f_col); l_na = frac_na(l_col); d_na = frac_na(d_col)
...
def build_labs_features(cohort_path: Path = None, save: bool = True) -> pd.DataFrame:
    """
    Build labs features from utils.LABS_PATH and cohort_path
    """
    LOG.info("Building labs features keyed by feature_name.")
    LOG.info("Loading inputs...")
    cohort_path = cohort_path or utils.FILTERED_COHORT_PATH
    labs_df, cohort_df, feature_names_all = _load_inputs(cohort_path)

    na_name_rows = int((labs_df["feature_name"].isna()).sum())
    if na_name_rows:
        LOG.warning("Rows with NA feature_name in LABS_PATH: %d (excluded from aggregations)", na_name_rows)

    LOG.info("Constraining labs to [0,48) and assigning windows...")
    df48 = _constrain_to_first_48h(labs_df, cohort_df)
    ...
    LOG.info("Forward-filling stats across windows within (subject_id,feature_name)...")
    long_ff = _ffill_stats_within_feature(long)

    LOG.info("Pivoting per-window features to wide...")
    ...
    LOG.info(f"Final labs feature matrix: rows={len(features):,}, cols={features.shape[1]:,}")
    ...
    LOG.info(f"Subjects with zero labs in 0–48h: {subj_zero:,}")
    LOG.info(f"Subjects with ≥1 lab measurement: {subj_nonzero:,}")
    _log_feature_summaries(features, feature_names_all)

    if save:
        utils.LABS_FEATURES_PATH.parent.mkdir(parents=True, exist_ok=True)
        features.to_parquet(utils.LABS_FEATURES_PATH, index=False)
        LOG.info(f"Wrote labs features to: {utils.LABS_FEATURES_PATH}")
