...
LOG = utils.make_logger("vitals_features", "vitals_features.log")
...
def _load_inputs(cohort_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    """
    Returns:
      vitals_df: columns (subject_id, admission_id, feature_name, charttime, valuenum)
      cohort_df: columns (subject_id, admission_id, admittime)
      feature_names_all: ordered unique feature names from vitals.csv with TempF converted to TempC.
    """
    def _apply_feature_alias(name: str | None) -> str | None:
        if name is None or pd.isna(name):
            return name
        return "TempC" if str(name) == "TempF" else str(name)

    # Base vitals table
    vitals_df = pd.read_parquet(utils.VITALS_PATH).loc[
    ...
    vitals_df["feature_name"] = vitals_df["feature_name"].astype("string").map(_apply_feature_alias)
    ...
    # feature names from VITALS_CSV
    csv = pd.read_csv(utils.VITALS_CSV)
    if "feature name" not in csv.columns:
        raise ValueError(f"{utils.VITALS_CSV} must have a 'feature name' column")

    feature_names_all = (
        csv["feature name"]
        .astype("string")
        .map(_apply_feature_alias)
        .drop_duplicates()
        .tolist()
    )

    return vitals_df, cohort_df, feature_names_all


def _constrain_to_first_48h(vitals_df: pd.DataFrame, cohort_df: pd.DataFrame) -> pd.DataFrame:
    df = vitals_df.merge(
...
def _compute_between_window_diffs(ffilled_long: pd.DataFrame, feature_names_all: list[str]) -> pd.DataFrame:
    """
    Using ffilled window means, compute for each feature_name and subject:
      - diff0/1/2 baseline_mean: w0_mean - w(i+1)_mean
      - diff0/1/2 previous_mean: w(i+1)_mean - w(i)_mean
      - diff0/1/2 sign_mean: {NEG, ZERO, POS}, with NA if `previous` is NA.
    """
...
def build_vitals_features(cohort_path: Path = None, save: bool = True) -> pd.DataFrame:
    """
    Build subject-level vitals features from utils.VITALS_PATH and chort_path
    """
    LOG.info("Building vitals features keyed by feature_name.")
    LOG.info("Loading inputs...")
    cohort_path = cohort_path or utils.FILTERED_COHORT_PATH
    vitals_df, cohort_df, feature_names_all = _load_inputs(cohort_path)

    # Report rows lacking feature_name (these will be ignored for aggregations)
    na_name_rows = int((vitals_df["feature_name"].isna()).sum())
    if na_name_rows:
        LOG.warning("Rows with NA feature_name in VITALS_PATH: %d (excluded from aggregations)", na_name_rows)

    LOG.info("Constraining vitals to [0,48) and assigning windows...")
    df48 = _constrain_to_first_48h(vitals_df, cohort_df)
    ...
    LOG.info(f"Final vitals feature matrix: rows={len(features):,}, cols={features.shape[1]:,}")
    ...
    LOG.info(f"Subjects with zero vitals in 0–48h: {subj_zero:,}")
    LOG.info(f"Subjects with ≥1 vital measurement: {subj_nonzero:,}")

    # Item/feature coverage per subject
    ...
    # top-N and NA-fraction breakdowns
    _log_feature_summaries(features, feature_names_all)

    if save:
        utils.VITALS_FEATURES_PATH.parent.mkdir(parents=True, exist_ok=True)
        features.to_parquet(utils.VITALS_FEATURES_PATH, index=False)
        LOG.info(f"Wrote vitals features to: {utils.VITALS_FEATURES_PATH}")
